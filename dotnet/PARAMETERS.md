...
| --- | --- | --- | --- | --- | --- |
| `BEYONDTRUST_ENABLED` | No | Both | `true` | Provider'ı enabled veya disabled yapar. Invalid boolean value validation error üretir. | `true` |
...
| `BEYONDTRUST_CERTIFICATE_CONTENT` | No | Both | - | PEM certificate content değeridir. | `-----BEGIN CERTIFICATE-----...` |
...
- Shared boolean parameter'lar `.NET` ve `Java` tarafında aynı success/error/default davranışını izler.
